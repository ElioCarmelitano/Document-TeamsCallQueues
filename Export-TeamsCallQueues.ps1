...
[CmdletBinding()]
param(
    [string]$OutputPath = (Join-Path (Get-Location) ("Teams-CallQueues-Report-{0}.html" -f (Get-Date -Format "yyyyMMdd-HHmmss"))),
    [switch]$Open
)
...
function Resolve-User {
    <#
      .SYNOPSIS
      Returns a friendly label for a user id/UPN; caches results.
    #>
    param($Id)
    $idStr = [string]$Id
    ...
    if (-not $script:Cache.Users.ContainsKey($idStr)) {
        try {
            ...
            $script:Cache.Users[$idStr] = ($u.DisplayName ?? $u.UserPrincipalName ?? $idStr)
        } catch {
...
function Resolve-Group {
    <#
      ...
      .NOTES
      Attempts Get-Team, then Get-UnifiedGroup, then falls back to raw id.
    #>
    param($Id)
    $idStr = [string]$Id
    if ([string]::IsNullOrWhiteSpace($idStr)) { return "" }
...
function Map-ActionLabel {
    <#
      .SYNOPSIS
      Normalizes CQ action enums to human labels.
    #>
    param([string]$Action)
    ...
    switch -Regex ($Action) {
        ...
        'Queue'             { "Queue" }
        'Transfer|Redirect' { "Forward/Transfer" }
        default             { $Action }
    }
}
